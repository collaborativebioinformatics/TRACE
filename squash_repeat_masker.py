#!/usr/bin/env python3
"""
squash_repeat_masker.py - Squash multiple rows per variant into single rows

This script processes BED files from RepeatMasker intersections and combines
multiple rows with the same SnifflesID into a single row, preserving
data from columns 2-9 with a '-' prefix for the missing first data column.
...
Example usage:
  ...
  %(prog)s input.bed --debug        # outputs to input_squashed.tsv with debug logging
    ...
    parser.add_argument(
        'input_file',
        help='Input BED file with RepeatMasker data'
    )
...
def process_bed_line(line: str, delimiter: str, line_num: int) -> Tuple[str, List[str]]:
    ...
    Returns:
        Tuple of (SnifflesID, list of columns 2-9 with '-' prefix)
    ...
    # Need at least 9 columns (0-8)
    if len(columns) < 9:
        logging.warning(f"Line {line_num}: Insufficient columns ({len(columns)} < 9), skipping")
        return None, None
    
    # SnifflesID is in column 1 (0-indexed column 0)
    sniffles_id = columns[0]
    
    # Extract columns 2-9 (0-indexed columns 1-8)
    # Prepend with '-' to indicate missing first data column
    elements = ['-'] + columns[1:9]
    ...
    try:
        with open(input_file, 'r') as f:
            for line_num, line in enumerate(f, 1):
                ...
                if line_num == 1 and (skip_header or line.startswith('#')):
